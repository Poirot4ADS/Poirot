...
bool FusionComponent::Init() {
  ...
  use_obstacles_gt = comp_config.use_obstacles_gt();
  ...
  obstacles_gt_reader = node_->CreateReader<PerceptionObstacles>("/apollo/custom/obstacles", nullptr);
...
bool FusionComponent::Proc(const std::shared_ptr<SensorFrameMessage>& message) {
  ...
  if (status) {
    ...
    if (message->sensor_id_ != fusion_main_sensor_) {
      ...
      obstacles_gt_reader->Observe();
      auto latest_msg = obstacles_gt_reader->GetLatestObserved();
      if(use_obstacles_gt && latest_msg && latest_msg->header().sequence_num() != 0)
      {
        out_message = latest_msg;
      }
...
bool FusionComponent::InternalProc(
  ...
  // valid_objects.clear();
  // viz_message->frame_->objects.clear();
  // AERROR << "clear1";
