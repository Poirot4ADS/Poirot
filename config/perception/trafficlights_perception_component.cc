...
bool TrafficLightsPerceptionComponent::Init() {
  ...
  tl_gt_reader = node_->CreateReader<TrafficLightDetection>("/apollo/custom/traffic_lights", nullptr);
...
int TrafficLightsPerceptionComponent::InitConfig() {
  ...
  use_tl_gt = traffic_light_param.use_tl_gt();
...
void TrafficLightsPerceptionComponent::OnReceiveImage(
  ...
  tl_gt_reader->Observe();
  auto latest_msg = tl_gt_reader->GetLatestObserved();
  if(use_tl_gt && latest_msg && latest_msg->header().sequence_num() != 0)
  {
    out_msg = latest_msg;
  }
  else{
