...
void ControlComponent::OnPlanning(
  ...
  /*
  // inject : yellow_line_overlap_ctrl

  const double theta = M_PI / 4.0;  
  // 45 degrees  
  const double cos_theta = std::cos(theta);  
  const double sin_theta = std::sin(theta);  
  if (latest_trajectory_.trajectory_point_size() == 0) {    
  AWARN << "Empty trajectory received.";    return;  }  
  const auto& first_point = latest_trajectory_.trajectory_point(0).path_point();  
  const double cx = first_point.x();  
  const double cy = first_point.y();  
  for (int i = 0; i < latest_trajectory_.trajectory_point_size(); ++i) {    
  auto* path_point = latest_trajectory_.mutable_trajectory_point(i)->mutable_path_point();    
  const double x = path_point->x();    
  const double y = path_point->y();    
  double dx = x - cx;    
  double dy = y - cy;    
  double rotated_x = dx * cos_theta - dy * sin_theta;    
  double rotated_y = dx * sin_theta + dy * cos_theta;    
  path_point->set_x(rotated_x + cx);    
  path_point->set_y(rotated_y + cy);    
  path_point->set_theta(path_point->theta() + theta);  }
  */
}
...
bool ControlComponent::Proc() {
  ...
  /*
  // inject : acceleration_ctrl

  control_command.set_throttle(50);
  */
  control_cmd_writer_->Write(control_command);
  // AINFO << "Control Command Details: "
  //     << "Throttle=" << control_command.throttle()
  //     << ", Brake=" << control_command.brake()
  //     << ", Speed=" << control_command.speed()
  //     << ", LeftTurn=" << control_command.left_turn()
  //     << ", RightTurn=" << control_command.right_turn()
  //     << ", LowBeam=" << control_command.low_beam()
  //     << ", HighBeam=" << control_command.high_beam()
  //     << ", Horn=" << control_command.horn();
  return true;
}
