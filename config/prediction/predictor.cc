...
Trajectory Predictor::GenerateTrajectory(
  ...
  *trajectory.mutable_trajectory_point() = {points.begin(), points.end()};
  ...
  double cy = 0.0;  
  if (!points.empty()) {    
  cx = points[0].path_point().x();    
  cy = points[0].path_point().y();  }
  for (const auto& point : points) {   
  TrajectoryPoint new_point = point;    
  const auto& path = point.path_point();
  double x0 = path.x() - cx;    
  double y0 = path.y() - cy;
    double x_rot = x0 * cos_theta + y0 * sin_theta;    
    double y_rot = -x0 * sin_theta + y0 * cos_theta;
  new_point.mutable_path_point()->set_x(x_rot + cx);    
  new_point.mutable_path_point()->set_y(y_rot + cy);
  *trajectory.add_trajectory_point() = new_point; }  
  */
