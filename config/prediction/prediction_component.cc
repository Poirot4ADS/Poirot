...
bool PredictionComponent::Init() {
  ...
  use_pred_gt = prediction_conf.topic_conf().use_pred_gt();
  pred_gt_reader = node_->CreateReader<PredictionObstacles>("/apollo/custom/prediction", nullptr);
...
bool PredictionComponent::PredictionEndToEndProc(
  ...
  pred_gt_reader->Observe();
  auto latest_msg = pred_gt_reader->GetLatestObserved();
  if(use_pred_gt && latest_msg && latest_msg->header().sequence_num() != 0)
  {
    prediction_obstacles = *latest_msg;
  }
  else
  {
  ...
  '''
  // inject: no_perception_pred

  std::shared_ptr<PerceptionObstacles> a = std::make_shared<PerceptionObstacles>();  
  a->mutable_header()->set_lidar_timestamp(perception_obstacles->header().lidar_timestamp());  
  a->mutable_header()->set_radar_timestamp(perception_obstacles->header().radar_timestamp());  
  a->mutable_header()->set_camera_timestamp(perception_obstacles->header().camera_timestamp());  
  a->set_error_code(perception_obstacles->error_code());  
  const std::shared_ptr<PerceptionObstacles>& b = a;  
  auto perception_msg = *b;
  '''
  ...
  prediction_obstacles.mutable_prediction_obstacle()->Clear();
  '''
