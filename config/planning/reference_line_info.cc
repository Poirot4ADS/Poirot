...
/*
// inject : yellow_line_overlap_plan
  const double theta = M_PI / 4.0;    
const double cos_theta = std::cos(theta);    
const double sin_theta = std::sin(theta); 
apollo::common::TrajectoryPoint first_point = discretized_trajectory_.StartPoint(); 
const double cx = first_point.path_point().x();  
const double cy = first_point.path_point().y(); 
 auto si = discretized_trajectory_.size();  
 for(size_t i = 0; i < si; ++i) 
  {    
  auto* path_point = discretized_trajectory_[i].mutable_path_point();   
...
bool ReferenceLineInfo::CombinePathAndSpeedProfile(
  ...
  for (double cur_rel_time = 0.0; cur_rel_time < speed_data_.TotalTime();
    ...
    /*
    // inject : combine_high_plan
    // trajectory_point.set_v(speed_point.v());
    // trajectory_point.set_a(speed_point.a());
    trajectory_point.set_v(5);
    trajectory_point.set_a(2);
    */
