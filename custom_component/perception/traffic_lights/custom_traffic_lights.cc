...
class TrafficLightsFolderReplayer {
 public:
  explicit TrafficLightsFolderReplayer(const std::string& folder)
      : folder_(folder) {}
  ...
  void Run() {
    ...
    std::string node_name = "traffic_lights" + std::to_string(getpid());
    auto node = apollo::cyber::CreateNode(node_name);
    auto writer = node->CreateWriter<TrafficLightDetection>(
        "/apollo/custom/traffic_lights");

    std::shared_ptr<TrafficLightDetection> msg = std::make_shared<TrafficLightDetection>();

    timer_ = std::make_unique<apollo::cyber::Timer>(100, [this, writer, msg]() {
      if (current_index_ >= sorted_files_.size()) {
        auto end_msg = std::make_shared<TrafficLightDetection>();
 ...
 private:
  ...
  bool LoadFromFile(const std::string& path, TrafficLightDetection* msg) {
...
int main(int argc, char** argv) {
  ...
  TrafficLightsFolderReplayer replayer(folder);
  replayer.Run();
