...
bool ControlComponent::Init() {
  ...
  result_fault_injection_ = control_conf_.result_fault_injection();
  if (result_fault_injection_)
  {
    throttle_offset_ = control_conf_.throttle_offset();
    brake_offset_ = control_conf_.brake_offset();
    steering_rate_offset_ = control_conf_.steering_rate_offset();
  }
...
bool ControlComponent::Proc() {
  ...
  if (result_fault_injection_)
  {
    if (throttle_offset_ != 0.0)
    {
      double new_throttle = std::max(0.0, control_command.throttle() + throttle_offset_);
      new_throttle = std::min(new_throttle, 100.0);
      control_command.set_throttle(new_throttle);
    }
    if (brake_offset_ != 0.0)
    {
      double new_brake = std::max(0.0, control_command.brake() + brake_offset_);
      new_brake = std::min(new_brake, 100.0);
      control_command.set_brake(new_brake);
    }
    if (steering_rate_offset_ != 0.0)
    {
      double new_steering_rate = std::max(0.0, control_command.steering_rate() + steering_rate_offset_);
      new_steering_rate = std::min(new_steering_rate, 100.0);
      control_command.set_steering_rate(new_steering_rate);
    }
  }
