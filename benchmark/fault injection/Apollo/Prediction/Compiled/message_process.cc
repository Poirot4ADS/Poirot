...
void MessageProcess::OnPerception(
    ...
    const std::unordered_map<int, int> &interactive,
  ...
  if (!interactive.empty() && !priority.empty())
  {   
      out_file << "Have priority and interactive" << std::endl;
      const auto& obstacle_ids =
      ptr_obstacles_container->curr_frame_movable_obstacle_ids();
      for (const int obstacle_id : obstacle_ids) 
      {

        if (interactive.find(obstacle_id) != interactive.end() && priority.find(obstacle_id) != priority.end())
        {
          Obstacle* obstacle_ptr = ptr_obstacles_container->GetObstacle(obstacle_id);
          ...
          // int int_priority = priority.at(obstacle_id);
          int int_interactive = interactive.at(obstacle_id);
          // auto lgsvl_priority = ObstaclePriority::CAUTION;
          auto lgsvl_interactive = ObstacleInteractiveTag::INTERACTION;
          /*
          if (int_priority == 0)
            lgsvl_priority = ObstaclePriority::IGNORE;
          else if (int_priority == 1)
            lgsvl_priority = ObstaclePriority::NORMAL;
          else
            lgsvl_priority = ObstaclePriority::CAUTION;
          */
          if (!int_interactive)
            lgsvl_interactive = ObstacleInteractiveTag::NONINTERACTION;
          else
            lgsvl_interactive = ObstacleInteractiveTag::INTERACTION;
          /*
          if (latest_feature_ptr->priority().priority() == ObstaclePriority::CAUTION)
            out_file << "Apollo's Priority is CAUTION" << std::endl;
          
          if (latest_feature_ptr->priority().priority() == ObstaclePriority::NORMAL)
            out_file << "Apollo's Priority is NORMAL" << std::endl;
          
          if (latest_feature_ptr->priority().priority() != lgsvl_priority)
            latest_feature_ptr->mutable_priority()->set_priority(lgsvl_priority);
          */
          
          if (latest_feature_ptr->interactive_tag().interactive_tag() != lgsvl_interactive)
            latest_feature_ptr->mutable_interactive_tag()->set_interactive_tag(lgsvl_interactive);
