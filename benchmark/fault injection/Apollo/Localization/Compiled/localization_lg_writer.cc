...
bool ReadFlagFromFile(const std::string& file_name) {
    std::string line;
...
void merge_and_save(double timestamp, const std::shared_ptr<Gps>& gps) {
    ...
    if (imu_it != imu_data.end() && corrected_imu_it != corrected_imu_data.end()) {
        ...
        if (config["use_ideal_module_data"] == "true")
        {
            InsStat gps_status;
...
int main(int argc, char* argv[]) {
    ...
    auto node = apollo::cyber::CreateNode("localization_gt");
    std::shared_ptr<apollo::cyber::Node> shared_node = std::move(node);
    ReadFlagFromFile("modules/localization/localization_ideal.txt");
    std::cout << "TF init!" << std::endl;
    tf2_broadcaster_.reset(new apollo::transform::TransformBroadcaster(shared_node));
    std::cout << "Init success!" << config["localization_topic"] << std::endl;
    localization_writer = shared_node->CreateWriter<LocalizationEstimate>(config["localization_topic"]);

    std::cout << "Creat reader!" << std::endl;
